...
class Cards extends React.Component {
  getData() {
    ...
    let totalSize = this.props.build[index].size || 0
    const chunk = this.props.build[index].chunks
    const chunksTotal = chunk.length
    const modulesTotal = chunk.reduce((sum, value) => {
      return sum + value.modules.length
    }, 0)
    ...
    if (index > 0) {
      const sizeDiff =
        ((totalSize - this.props.build[index - 1].size) / totalSize) * 100
    ...
    if (totalSize.toString().length > 3 && totalSize.toString().length < 7)
      totalSize = `${Math.floor(totalSize / 1000)}KB`
    if (totalSize.toString().length > 6)
      totalSize = `${(totalSize / 1000000).toFixed(2)}MB`
    ...
    const indexData = (
      <span>
        <span
          ...
          className={
            'arrow ' + (this.props.activeBuild === 0 ? 'inactive' : '')
          }
        >
          &#9666;
        </span>
        ...
        <span
          ...
          className={
            'arrow ' +
            (this.props.activeBuild === this.props.build.length - 1
              ? 'inactive'
              : '')
          }
        >
